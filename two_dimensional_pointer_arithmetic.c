...
int* get_element_ptr_flat(int *base_ptr, int row, int col, int cols_per_row) {
    return base_ptr + row * cols_per_row + col;
}

int main(void) {
    int matrix[2][3] = { {1, 2, 3}, {10, 20, 30} };
    int *base_ptr = (int *)matrix;

    int *ptr1 = get_element_ptr(matrix, 1, 2); // matrix[1][2]
    int *ptr2 = get_element_ptr_flat(base_ptr, 1, 1, 3); // matrix[1][1]
